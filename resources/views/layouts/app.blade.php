        ...
        @auth
            <aside class="sidebar">
                <div class="sidebar-brand">
                    <i data-lucide="layers" style="margin-right: 10px; width: 24px; height: 24px;"></i> KÖKSAN DOKÜMAN YÖNETİM SİSTEMİ
                </div>
                <ul class="sidebar-nav">
                    <li>
                        <a href="{{ route('dashboard') }}" class="{{ request()->routeIs('dashboard') ? 'active' : '' }}">
                            <i data-lucide="layout-dashboard" class="nav-icon"></i> Gösterge Paneli
                        </a>
                    </li>

                    <li class="nav-section">DOKÜMANLAR</li>
                    <li>
                        <a href="{{ route('documents.index') }}"
                            class="{{ request()->routeIs('documents.index', 'documents.show') ? 'active' : '' }}">
                            <i data-lucide="folder-search" class="nav-icon"></i> Tüm Belgeler
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('documents.create') }}"
                            class="{{ request()->routeIs('documents.create') ? 'active' : '' }}">
                            <i data-lucide="upload-cloud" class="nav-icon"></i> Yeni Belge Yükle
                        </a>
                    </li>

                    @can('document.create')
                        <li class="nav-section">SİSTEM YÖNETİMİ</li>
                        <li>
                            <a href="{{ route('folders.index') }}"
                                class="{{ request()->routeIs('folders.*') ? 'active' : '' }}">
                                <i data-lucide="folder-tree" class="nav-icon"></i> Klasör Yönetimi
                            </a>
                        </li>
                        @hasanyrole('Super Admin|Admin')
                            <li>
                                <a href="{{ route('settings.permissions') }}"
                                    ...
                                    <i data-lucide="shield-alert" class="nav-icon"></i> 3D Yetki Matrisi
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('settings.notifications') }}"
                                    class="{{ request()->routeIs('settings.notifications') ? 'active' : '' }}">
                                    <i data-lucide="printer" class="nav-icon"></i> Otomatik Rapor Ayarları
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('settings.mail') }}"
                                    class="{{ request()->routeIs('settings.mail') ? 'active' : '' }}">
                                    <i data-lucide="mail" class="nav-icon"></i> Mail Şablonları ve Ayarlar
                                </a>
                            </li>
                        @endhasanyrole
                    @endcan
                    ...
                    @can('user.manage')
                        <li>
                            <a href="{{ route('users.index') }}" class="{{ request()->routeIs('users.*') ? 'active' : '' }}">
                                <i data-lucide="users" class="nav-icon"></i> Kullanıcı Yönetimi
                            </a>
                        </li>
                    @endcan
                </ul>
                            ...
                            <div id="notificationDropdown" class="notification-dropdown glass-card">
                                    ...
                                    @forelse(auth()->user()->unreadNotifications->take(5) as $notification)
                                        <a href="{{ $notification->data['url'] ?? '#' }}" class="notification-item unread">
                                            ...
                                            <div class="notif-content">
                                                <div class="notif-title">{{ $notification->data['title'] ?? 'Bildirim' }}
                                                </div>
                                                <div class="notif-desc">{{ $notification->data['message'] ?? '' }}</div>
                                ...
                                <div class="dropdown-footer flex-between"
                                    ...
                                    <a href="{{ route('profile.notifications') }}"
                                        ...
                                        <i data-lucide="settings" style="width: 14px; height: 14px;"></i> Ayarlar
                                    </a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="user-dropdown-container relative-container" style="position: relative;">
                            ...
                            <div id="userDropdownMenu" class="dropdown-menu glass-card"
                                ...
                                <a href="{{ route('profile.edit') }}"
                                    ...
                                    <i data-lucide="settings" style="width: 18px; height: 18px;"></i> Profilimi Düzenle
                                </a>
                                <a href="{{ route('profile.delegations') }}"
                                    style="display: flex; align-items: center; gap: 10px; padding: 12px; color: var(--text-color); text-decoration: none; border-bottom: 1px solid var(--border-color); border-radius: 6px; transition: background 0.2s;">
                                    <i data-lucide="users" style="width: 18px; height: 18px;"></i> Vekalet İşlemleri
                                </a>
