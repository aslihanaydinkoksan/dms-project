...
@section('content')
    <div class="explorer-breadcrumb flex-between mb-20"
        ...
        <div class="breadcrumb-path"
            ...
            <a href="{{ route('folders.index') }}" class="crumb-link"
                ...
                <i data-lucide="home" style="width: 18px;"></i> Ana Dizin
            </a>
        ...
        <div class="explorer-actions" style="display: flex; gap: 10px;">
            @if (auth()->user()->hasAnyRole(['Super Admin', 'Admin']) || auth()->id() === 1)
                <a href="{{ route('folders.edit', $folder->id) }}" class="btn btn-outline-secondary"
                    ...
                    <i data-lucide="settings" style="width: 18px; color: var(--text-muted);"></i> Düzenle
                </a>
            @endif
            @can('delete', $folder)
                <form action="{{ route('folders.destroy', $folder->id) }}" method="POST" class="inline-form"
                    ...
                    <button type="submit" class="btn btn-outline-danger"
                        ...
                        <i data-lucide="trash-2" style="width: 18px;"></i> Klasörü Sil
                    </button>
                </form>
            @endcan
            ...
            <button class="btn btn-outline-primary" onclick="document.getElementById('newFolderModal').style.display='flex'"
                ...
                <i data-lucide="folder-plus" style="width: 18px;"></i> Yeni Klasör
            </button>

            <a href="{{ route('documents.create', ['folder_id' => $folder->id]) }}" class="btn btn-primary"
                style="display: flex; align-items: center; gap: 8px; padding: 10px 15px;">
                <i data-lucide="file-up" style="width: 18px;"></i> Belge Yükle
            </a>
        </div>
    </div>
    ...
    @if ($folder->department_id)
        <div class="alert alert-info flex"
            ...
            <span><strong>İzole Klasör:</strong> Bu klasör ve içindekiler sadece
                <strong>{{ $folder->department->name }}</strong> departmanına aittir.</span>
        </div>
    @endif

    <div class="file-explorer-container mt-20">
                    ...
                    <a href="{{ route('folders.show', $sub->id) }}" class="explorer-item folder-item"
                        ...
                        <div class="item-details" style="flex: 1;">
                            ...
                            <div class="item-meta" style="font-size: 0.8rem; color: var(--text-muted);">
                                {{ $sub->children->count() }} Alt Klasör</div>
                        </div>
        ...
        <div
            ...
            <h3 class="explorer-section-title" style="margin: 0; font-size: 1.1rem; color: var(--secondary-color);">Belgeler
            </h3>
        </div>
                        ...
                        <div class="item-details" style="flex: 1;">
                            ...
                            <div class="item-meta"
                                ...
                                <span><i data-lucide="user" style="width: 10px; display: inline-block;"></i>
                                    {{ $doc->currentVersion?->createdBy?->name ?? 'Bilinmiyor' }}</span>
                            </div>
                        </div>
        ...
        @else
            <div class="empty-state text-center"
                ...
                <p class="text-muted" style="margin: 0; font-size: 1.05rem;">Bu klasörde henüz belge bulunmuyor.</p>
            </div>
        @endif
    </div>
        ...
        <div class="modal-content"
            ...
            <div class="modal-header"
                ...
                <h2
                    ...
                    <i data-lucide="corner-down-right" style="color: var(--accent-color);"></i> Alt Klasör Oluştur
                </h2>
            ...
            <div class="modal-body" style="padding: 25px;">
                <form action="{{ route('folders.store') }}" method="POST" class="modern-form">
                    ...
                    <div class="form-group" style="margin-bottom: 20px;">
                        <label
                            style="display: block; font-size: 0.85rem; font-weight: 600; margin-bottom: 8px; color: var(--text-muted);">Hedef
                            Dizin (Üst Klasör)</label>
                        <div
                    ...
                    <div class="form-group" style="margin-bottom: 25px;">
                        <label
                            style="display: block; font-size: 0.9rem; font-weight: 600; margin-bottom: 8px; color: var(--secondary-color);">Yeni
                            Alt Klasör Adı <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control" required autofocus
                            placeholder="Örn: 2026 Bütçe Raporları"
                    ...
                    <div class="form-group" style="margin-top: 15px;">
                        <label
                            ...
                            Öneki (Opsiyonel)</label>
                        <input type="text" name="prefix" class="form-control" placeholder="Örn: IK veya TL"
                            style="width: 100%; padding: 12px; border: 1px solid var(--border-color); border-radius: 6px;">
                        <small class="text-muted">Ana klasöre IK, alt klasöre TL yazarsanız belge IK-TL-001 olur.</small>
                    </div>
                    ...
                    @if ($folder->department_id)
                        <div class="alert alert-warning"
                            ...
                            <span style="color: #0f766e;">
                                <strong>Güvenlik Mirası:</strong> Bu alt klasör, üst klasörün yetki kurallarını miras alacak
                                ve sadece <strong>{{ $folder->department->name }}</strong> departmanı tarafından
                    ...
                    <div class="form-actions"
                        ...
                        <button type="submit" class="btn btn-primary"
                            ...
                            <i data-lucide="folder-plus" style="width: 16px;"></i> Alt Klasörü Oluştur
                        </button>
                    </div>
                </form>
            </div>
