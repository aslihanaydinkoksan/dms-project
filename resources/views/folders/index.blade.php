...
@section('content')
    <div class="explorer-breadcrumb flex-between mb-20"
        ...
        <div class="breadcrumb-path" style="display: flex; align-items: center; gap: 10px;">
            ...
            <span class="crumb-current"
                style="font-size: 1.25rem; font-weight: 700; color: var(--primary-color);">{{ __('Ana Dizin (Kök Klasörler)') }}</span>
        </div>
        ...
        <div class="explorer-actions">
            @if (auth()->user()->hasAnyRole(['Super Admin', 'Admin']) || auth()->id() === 1)
                <button class="btn btn-primary" onclick="document.getElementById('newRootFolderModal').style.display='flex'"
                    ...
                    <i data-lucide="folder-plus" style="width: 18px;"></i> {{ __('Yeni Ana Klasör Oluştur') }}
                </button>
            @endif
        </div>
    </div>
    ...
    <div class="alert alert-info mb-20 flex"
        ...
        <div>
            <strong>{{ __('Dosya Yöneticisi:') }}</strong>
            {{ __('Aşağıda erişim yetkiniz olan ana dizinleri görmektesiniz. Alt klasörleri ve belgeleri görmek için bir klasörün içine tıklayın.') }}
        </div>
    </div>
    ...
    <div class="file-explorer-container">
                        ...
                        <a href="{{ route('folders.show', $folder->id) }}"
                            ...
                            @if ($folder->departments->count() > 0)
                                <div class="item-badge"
                                    ...
                                    {{ $folder->departments->count() }} {{ __('DEPARTMAN') }}
                                </div>
                            @else
                                <div class="item-badge"
                                ...
                                <div class="item-details" style="flex: 1; min-width: 0;">
                                    ...
                                    <div class="item-meta"
                                        ...
                                        <span style="display: flex; align-items: center; gap: 6px;"><i
                                            ...
                                            {{ $folder->children()->count() }} {{ __('Alt Klasör') }}</span>
                                        <span style="display: flex; align-items: center; gap: 6px;"><i
                                                data-lucide="file-text" style="width: 14px;"></i>
                                            {{ $folder->documents()->count() }} {{ __('Belge') }}</span>
                                    </div>
                                </div>
                            </div>
                        </a>
        ...
        @else
            <div class="empty-state text-center"
                ...
                <h3 style="color: var(--primary-color); margin-bottom: 10px;">{{ __('Ana Dizin Bomboş') }}</h3>
                <p class="text-muted">{{ __('Sistemde erişim yetkiniz olan hiçbir kök klasör bulunmuyor.') }}</p>
            </div>
        @endif
    </div>
        ...
        <div class="modal-content"
            ...
            <div class="modal-header"
                ...
                <h2 style="margin: 0; font-size: 1.25rem; display: flex; align-items: center; gap: 8px;">
                    <i data-lucide="folder-plus" style="color: var(--accent-color);"></i>
                    {{ __('Ana Dizin (Kök Klasör) Oluştur') }}
                </h2>
            ...
            <div class="modal-body" style="padding: 25px;">
                <form action="{{ route('folders.store') }}" method="POST" class="modern-form">
                    ...
                    <div class="form-group" style="margin-bottom: 20px;">
                        <label
                            style="display: block; font-size: 0.9rem; font-weight: 600; margin-bottom: 8px; color: var(--secondary-color);">{{ __('Klasör Adı') }}
                            <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control" required autofocus
                            placeholder="{{ __('Örn: İnsan Kaynakları') }}"
                    ...
                    <div class="form-group" style="margin-top: 15px;">
                        ...
                        <input type="text" name="prefix" class="form-control" placeholder="{{ __('Örn: TL-IG') }}"
                            style="width: 100%; padding: 12px; border: 1px solid var(--border-color); border-radius: 6px; font-size: 1rem;">
                        <small class="text-muted" style="display: block; margin-top: 5px; font-size: 0.8rem;">
                            {{ __('Boş bırakılırsa, sistem otomatik olarak bir üst klasörün önekini kullanır (Miras Alma).') }}
                        </small>
                    </div>

                    <div class="form-group" style="margin-bottom: 25px;">
                        <label
                            style="display: block; font-size: 0.9rem; font-weight: 600; margin-bottom: 8px; color: var(--secondary-color);">{{ __('Erişim Kısıtlaması (Departman Seçimi)') }}</label>
                        ...
                        <div class="alert alert-warning"
                            ...
                            <span style="color: #92400e;"><strong>{{ __('İpucu:') }}</strong>
                                {{ __('Hiçbirini seçmezseniz klasör "Herkese Açık" olur. Birden fazla seçerseniz departmanlara ortak paylaştırılır.') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="form-actions"
                        ...
                        <button type="submit" class="btn btn-primary"
                            ...
                            <i data-lucide="save" style="width: 16px;"></i> {{ __('Klasörü Oluştur') }}
                        </button>
                    </div>
                </form>
            </div>
